...
- **THEN** the browser downloads a video file with frame dimensions equal to composition width × height (or the nearest even size)The exported video MUST contain exactly one loop of the configured animation period at the configured frame rate, with phase continuous so looping playback in Resolume does not jump.
